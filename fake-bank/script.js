...
const displayMovements = function(acc, sort = false){
  ...
  movs.forEach(function(mov, i){
    ...
    <div class="movements__value">${formattedMov(mov, acc.locale, acc.currency)}</div>
    </div>`
...
const calcDisplayBalance = function(acc) {
  ...
  labelBalance.textContent = formattedMov(acc.balance, acc.locale, acc.currency);
};
...
btnTransfer.addEventListener('click', function(e) {
  ...
  // check money > 0  && person has enough money && 받는 사람이 나와 같지 않으면서 존재한다면
